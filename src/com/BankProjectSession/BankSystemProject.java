package com.BankProjectSession;
...
public class BankSystemProject {
		...
		void register() {
			...
			while(flag != 1) {
				...
				if(Pattern.matches("[0-9]{10}", contact)) {
					if(phone.indexOf(contact) < 0) {
						...
						System.out.println("This mobile number already exist. Reset: ");
					}
				}
				else {
					System.out.println("Please enter only 10 integers.Reset:");
				}
			}
			...
			while(flag != 1) {
				...
				if(uname.indexOf(u) < 0) {
					...
					System.out.println("Username already exist .Reset: ");
				}	
			}
		...
		void deposit(int i, double amt) {
			String DStat = "Rs. "+amt+" Credited to your account. Remaining Balance: ";
			amt += amount.get(i);
...
//			System.out.println(transaction);
		...
		public static void main(String[] args) {
			BankSystemProject bankObj = new BankSystemProject();
			int choice = 0;
